...
melt = melt_file('sep_2019.xlsx')

def day_filter(df, date):
    '''
...
day_filter = day_filter(melt, '2019-09-01')

def desk_filter(df, desk):
    '''
    Def: Filters desk by desk names and day
    Inputs:
        df: full melted dataframe
    ...
    df_date_desk = df[df['Desk'].isin(desk)]

    return df_date_desk

desk_filter = desk_filter(day_filter, ['3'])
